...
- [Variant inventory](project_airtable-inventory.md) — per-variant stock (now Supabase `inventory_by_slug`); missing key ⇒ in stock, 0 ⇒ out, 1–5 ⇒ low; empty-map fallback when down.\n- [Supabase read-path migration](project_supabase-read-path-migration.md) — read path on Supabase since 2026-09-06; write path still Airtable; never use a cookie-bound client in the storefront.
